...
class AgentLoop:
    ...
    def __init__(self, max_iterations: int = 10, short_term_memory: Optional[Any] = None, max_tool_calls: int = 10):
        """
        初始化Agent循环引擎
                ...
                try:
                    ...
                    if llm_response.has_tool_calls():
                        ...
                        if self.tool_call_count >= self.max_tool_calls:
                            force_count = getattr(self, '_force_count', 0) + 1
                            self._force_count = force_count
                            if force_count > 2:
                                # 已强制2轮还不出结果，直接断
                                logger.warning(f"LLM still requesting tools after {force_count} force attempts, breaking")
                                break
                            logger.warning(f"已达到最大 Skill 调用次数 ({self.max_tool_calls})，强制生成最终答案 (第{force_count}次)")
                            messages.append({
                                'role': 'user',
                                'content': '请基于以上已获取的所有信息，立即给出最终答复。不要再调用工具。'
                            })
                            continue
                        self._force_count = 0  # 重置
                        ...
                        for tool_call in llm_response.tool_calls:
                            ...
                            if self.tool_call_count > self.max_tool_calls:
                                # 跳过执行但必须补一条假结果，否则 API 报 insufficient tool messages
                                logger.warning(f"跳过 {tool_call.name}（已达上限 {self.max_tool_calls}）")
                                messages.append(agent.llm_client.create_tool_message(
                                    tool_call_id=tool_call.id,
                                    tool_name=tool_call.name,
                                    result={"skipped": True, "reason": "已达最大调用次数限制"}
                                ))
                                continue
    ...
    def _create_assistant_message_with_tools(self, llm_response: LLMResponse) -> Dict[str, Any]:
        ...
        # 豆包/DeepSeek thinking 模式：必须回传 reasoning_content
            ...
            message['tool_calls'] = [
                {
                    ...
                    'function': {
                        'name': tc.name,
                        'arguments': json.dumps(tc.arguments, ensure_ascii=False).encode('utf-8', errors='replace').decode('utf-8')
                    }
                }
                for tc in llm_response.tool_calls
            ]
